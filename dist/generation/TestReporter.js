...
class TestReporter {
    ...
    async generateReport(testFiles, userPaths, validationResults) {
        ...
        try {
            ...
            logger_1.logger.info('Test report generated successfully', {
            ...
            logger_1.logger.error('Failed to generate test report', error);
            throw error;
        }
    }
    async generateFileReport(testFile, validationResult) {
    ...
    async exportToHtml(report) {
        ...
        logger_1.logger.info('HTML report saved', { path: outputPath });
        return outputPath;
    }
    async exportToJson(report) {
        ...
        logger_1.logger.info('JSON report saved', { path: outputPath });
        return outputPath;
    }
    async exportToMarkdown(report) {
        ...
        logger_1.logger.info('Markdown report saved', { path: outputPath });
        return outputPath;
    }
    async exportToXml(report) {
        ...
        logger_1.logger.info('XML report saved', { path: outputPath });
        return outputPath;
    }
    ...
    generateSummary(testFiles, validationResults) {
        ...
        const averageTestLength = validationResults.reduce((sum, r) => sum + r.metrics.averageTestLength, 0) /
            validationResults.length;
        const complexityScore = validationResults.reduce((sum, r) => sum + r.metrics.complexityScore, 0) /
            validationResults.length;
        const maintainabilityIndex = validationResults.reduce((sum, r) => sum + r.metrics.maintainabilityIndex, 0) /
            validationResults.length;
        return {
            totalFiles: testFiles.length,
            totalTests,
    ...
    async generateTestFileReports(testFiles, validationResults) {
        const reports = [];
    ...
    async generateCoverageSummary(testFiles, userPaths) {
    ...
    generatePerformanceSummary(testFiles) {
    ...
    generateRecommendations(testFiles, validationResults) {
        ...
        const avgMaintainability = validationResults.reduce((sum, r) => sum + r.metrics.maintainabilityIndex, 0) /
            validationResults.length;
        ...
        const hasFragileSelectors = validationResults.some((r) => r.warnings.some((w) => w.message.includes('Fragile selector')));
...
<testReport>
    ...
    calculateQualityGrade(maintainability, complexity) {
    ...
    calculateTestCoverage(testFile) {
