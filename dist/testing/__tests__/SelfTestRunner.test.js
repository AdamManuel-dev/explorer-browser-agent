...
describe('SelfTestRunner', () => {
    ...
    let tempTestDir;
    beforeEach(() => {
        tempTestDir = (0, path_1.join)((0, os_1.tmpdir)(), 'selftest-runner-test', Date.now().toString());
        (0, fs_1.mkdirSync)(tempTestDir, { recursive: true });
        testRunner = new SelfTestRunner_1.SelfTestRunner({
    ...
    afterEach(() => {
        try {
            if ((0, fs_1.existsSync)(tempTestDir)) {
                (0, fs_1.rmSync)(tempTestDir, { recursive: true, force: true });
            }
        }
        catch (error) {
    ...
    describe('Configuration', () => {
        it('should use default configuration when none provided', () => {
            const defaultRunner = new SelfTestRunner_1.SelfTestRunner();
            expect(defaultRunner).toBeDefined();
        });
        it('should merge provided configuration with defaults', () => {
            const customRunner = new SelfTestRunner_1.SelfTestRunner({
                testTimeout: 10000,
                retryAttempts: 3,
            });
    ...
    describe('Component Tests', () => {
        it('should run config manager test', async () => {
            const runner = new SelfTestRunner_1.SelfTestRunner({
    ...
    describe('Test Reporting', () => {
        ...
        it('should calculate correct summary statistics', async () => {
            ...
            expect(report.summary.passedTests + report.summary.failedTests + report.summary.skippedTests).toBe(report.summary.totalTests);
    ...
    describe('Error Handling', () => {
        it('should handle test timeouts gracefully', async () => {
            const timeoutRunner = new SelfTestRunner_1.SelfTestRunner({
                skipBrowserTests: true,
        ...
        it('should retry failed tests according to configuration', async () => {
            const retryRunner = new SelfTestRunner_1.SelfTestRunner({
                skipBrowserTests: true,
                testTimeout: 5000,
        ...
        it('should continue running tests even if some fail', async () => {
            const partialFailureRunner = new SelfTestRunner_1.SelfTestRunner({
                skipBrowserTests: true,
                testTimeout: 5000,
                retryAttempts: 0,
                componentTests: {
    ...
    describe('Configuration Validation', () => {
        it('should skip tests when components are disabled', async () => {
            const selectiveRunner = new SelfTestRunner_1.SelfTestRunner({
                skipBrowserTests: true,
                componentTests: {
                    config: true,
                    crawler: false,
        ...
        it('should respect browser test skip flag', async () => {
            const noBrowserRunner = new SelfTestRunner_1.SelfTestRunner({
                skipBrowserTests: true,
                componentTests: {
                    config: true,
                    crawler: false,
            ...
            const browserTests = report.results.filter((r) => r.name.includes('Basic Crawling') ||
...
//# sourceMappingURL=SelfTestRunner.test.js.map