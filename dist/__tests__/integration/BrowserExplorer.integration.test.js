...
describe('BrowserExplorer Integration Tests', () => {
    ...
    beforeAll(async () => {
        ...
        const tempDir = (0, path_1.join)((0, os_1.tmpdir)(), 'browser-explorer-test');
        testConfigPath = (0, path_1.join)(tempDir, 'test-config.yaml');
        testOutputDir = (0, path_1.join)(tempDir, 'test-output');
...
monitoring:
  ...
  alerting:
    ...
    thresholds:
      ...
      responseTime: 5000
`;
        (0, fs_1.writeFileSync)(testConfigPath, testConfig);
    ...
    afterAll(async () => {
        ...
        try {
            (0, fs_1.rmSync)((0, path_1.join)((0, os_1.tmpdir)(), 'browser-explorer-test'), { recursive: true, force: true });
        }
        catch (error) {
    ...
    beforeEach(async () => {
        ...
        const app = express();
        app.use(express.static((0, path_1.join)(__dirname, 'fixtures')));
        // Basic test pages
        app.get('/', (req, res) => {
            res.send(`
        <!DOCTYPE html>
...
`;
            (0, fs_1.writeFileSync)(limitedConfig, limitedConfigContent);
            const explorer = new index_1.BrowserExplorer();
            try {
    ...
    describe('Component Integration', () => {
        ...
        it('should integrate stealth mode correctly', async () => {
            ...
            const stealth = new stealth_1.StealthMode({
                enabled: true,
                fingerprintSpoofing: {
                    canvas: true,
                    webgl: true,
        ...
        it('should integrate CAPTCHA handler correctly', async () => {
            const page = await browser.newPage();
            const captchaHandler = new captcha_1.CaptchaHandler({
                autoDetect: true,
                solveAttempts: 2,
                timeout: 5000,
                manualSolving: {
        ...
        it('should integrate session manager correctly', async () => {
            ...
            try {
                ...
                const restoreResult = await sessionManager.restoreSessionToPage(newPage, sessionId, 'localhost');
...
generation:
  ...
  outputDirectory: "${testOutputDir}"
`;
            (0, fs_1.writeFileSync)(timeoutConfig, timeoutConfigContent);
            const explorer = new index_1.BrowserExplorer();
            try {
    ...
    describe('Performance and Scalability', () => {
        it('should handle multiple concurrent operations', async () => {
            ...
            const operations = [];
            for (let i = 0; i < 10; i++) {
                const operation = monitoring.timeFunction(`concurrent_operation_${i}`, async () => {
...
//# sourceMappingURL=BrowserExplorer.integration.test.js.map