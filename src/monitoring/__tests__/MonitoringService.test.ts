...
describe('MonitoringService', () => {
  ...
  describe('Metrics Collection', () => {
    ...
    it('should record histogram metrics', () => {
      ...
      expect((metrics['test_histogram'][0] as any).buckets).toEqual(buckets);
    });
    ...
    it('should record timer metrics', () => {
      ...
      expect((metrics['test_timer'][0] as any).duration).toBe(123.45);
    });
  ...
  describe('Timer Utilities', () => {
    it('should provide timer function', () => {
      ...
      setTimeout(() => {
        ...
        expect((metrics['test_timer_func'][0] as any).duration).toBeGreaterThan(0);
      }, 10);
    });
    ...
    it('should time async functions successfully', async () => {
      ...
      const result = await monitoringService.timeFunction(
        'async_test',
        mockAsyncFunction,
        { operation: 'test' }
      );
    ...
    it('should time async functions with errors', async () => {
      ...
      await expect(
        monitoringService.timeFunction('async_error_test', mockAsyncFunction)
  ...
  describe('Distributed Tracing', () => {
    ...
    it('should respect sampling rate', () => {
      const lowSamplingService = new MonitoringService({
        tracing: {
          enabled: true,
  ...
  describe('Reporting and Alerts', () => {
    ...
    it('should generate alerts based on thresholds', async () => {
      ...
      const alertService = new MonitoringService({
        alerting: {
          enabled: true,
          thresholds: {
            errorRate: 0.01, // 1%
  ...
  describe('Service Lifecycle', () => {
    it('should initialize and shutdown properly', async () => {
      const service = new MonitoringService({
        reporting: { enabled: false }, // Disable intervals for testing
      });
