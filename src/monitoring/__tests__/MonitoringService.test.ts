...
describe('MonitoringService', () => {
  ...
  describe('Metrics Collection', () => {
    ...
    it('should record histogram metrics', () => {
      ...
      expect((metrics.test_histogram?.[0] as { buckets?: number[] })?.buckets).toEqual(buckets);
    });
    ...
    it('should record timer metrics', () => {
      ...
      expect((metrics.test_timer?.[0] as { duration?: number })?.duration).toBe(123.45);
    });
  ...
  describe('Timer Utilities', () => {
    it('should provide timer function', (done) => {
      ...
      setTimeout(() => {
        ...
        expect((metrics.test_timer_func?.[0] as { duration?: number })?.duration).toBeGreaterThan(
          0
        );
        done();
      }, 10);
